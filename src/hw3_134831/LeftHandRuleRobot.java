...
public class LeftHandRuleRobot extends Robot {
    ...
    void move(Maze m) {
        ...
        while (!position.equals(m.getGoal()) && counter < 100 && !found) {
            ...
            if (direction.equals("north")) {
                ///if the point in front is not an obstacle and the point in left is an obstacle
                if (!isObstacle(new Point(x - 1, y)) && isObstacle(new Point(x, y - 1))) {
                    ...
                    //if there is no obstacle in the left of the robot
                } else if (!isObstacle(new Point(x, y - 1))) {
                    direction = "west"; //set the direction to west
                    position.setLocation(x, y - 1); //move forward of west
                    path.add(new Point(position)); //add the position to the path

                    //if there is an obstacle in front and an obstacle in left (corner)
    ...
    public String getPath() {
        String output = "Followed path by Left Hand Rule robot:\n";
